...
class _ReportScreenState extends State<ReportScreen> {
  ...
  Future<void> _loadReports() async {
    ...
    try {
      ...
      for (final o in _rawOrders) {
        ...
        if (o['status'] == AppConstants.statusDelivered) {
  ...
  Future<void> _exportMonthlyPDF() async {
    try {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Generating PDF...'), backgroundColor: AppColors.primary),
      );
      ...
      for (final o in selectedMonthOrders) {
        ...
        if (status == AppConstants.statusCancelled) serviceDetails[sId]!['cancelled'] += 1;
        if (status == AppConstants.statusPickedUp || status == AppConstants.statusOutForDelivery || status == 'dropped') {
            ...
            return pw.Column(
              ...
              children: [
                ...
                pw.TableHelper.fromTextArray(
                  ...
                  headerHeight: 28,
                  cellHeight: 24,
                  cellAlignments: {0: pw.Alignment.centerLeft, 1: pw.Alignment.center, 2: pw.Alignment.center, 3: pw.Alignment.center, 4: pw.Alignment.center, 5: pw.Alignment.center, 6: pw.Alignment.centerRight},
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to generate PDF: $e'), backgroundColor: AppColors.error));
    }
  }

  // PDF Helpers
  final List<PdfColor> _pdfChartColors = [
    PdfColor.fromHex('#4F46E5'), PdfColor.fromHex('#10B981'), PdfColor.fromHex('#F59E0B'), PdfColor.fromHex('#EC4899'),
    PdfColor.fromHex('#8B5CF6'), PdfColor.fromHex('#06B6D4'), PdfColor.fromHex('#F43F5E'), PdfColor.fromHex('#14B8A6'),
  ];
  ...
  pw.Widget _buildPdfPieChart(String title, bool isOrderVolume) {
    ...
    return pw.Column(
        ...
        children: [
          ...
          pw.Row(
            children: [
              pw.SizedBox(height: 80, width: 80, child: pw.Chart(grid: pw.PieGrid(), datasets: datasets)),
              pw.SizedBox(width: 15),
              pw.Expanded(child: pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: legendItems)),
            ],
          ),
        ]
    );
  ...
  pw.Widget _buildPdfSummaryBox(String title, String value, String hexAccent) {
    return pw.Container(
        width: 240,
        decoration: pw.BoxDecoration(color: PdfColor.fromHex('#F8FAFC'), borderRadius: const pw.BorderRadius.all(pw.Radius.circular(6)), border: pw.Border.all(color: PdfColor.fromHex('#E2E8F0'), width: 0.5)),
        child: pw.ClipRRect(
            ...
            child: ListView(
              ...
              children: [
                ...
                GridView.count(
                  ...
                  children: [
                    _buildKpiBox('Revenue', '৳${revenue.toStringAsFixed(0)}', Icons.attach_money, AppColors.success),
                    _buildKpiBox('Orders', '$orders', Icons.receipt_long_outlined, AppColors.primary),
                    _buildKpiBox('Avg Value', '৳${avgValue.toStringAsFixed(0)}', Icons.bar_chart, Colors.blue),
                    _buildKpiBox('Delivered', '$delivered', Icons.check_circle_outline, AppColors.success),
                  ],
                ),
                const SizedBox(height: 24),
                ...
                if (_filteredByService.isNotEmpty) ...[
                  _buildPieChartCard('Order Volume Ratio', true),
                  const SizedBox(height: 16),
                  _buildPieChartCard('Average Ratings Ratio', false),
                  const SizedBox(height: 32),
                ],

                // Services List
                Text('Orders by Service', style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.text)),
                ...
                if (_filteredByService.isEmpty)
                  Container(
                    ...
                    child: Center(child: Text('No orders found for this month.', style: GoogleFonts.inter(color: AppColors.subtext))),
                  )
                else
                  ..._filteredByService.map((s) {
                    ...
                    return Container(
                      ...
                      decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.border), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 4, offset: const Offset(0, 2))]),
                      child: ListTile(
                        ...
                        title: Row(
                          ...
                          children: [
                            Expanded(child: Text(s['title'] as String, style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 15, color: AppColors.text))),
                        ...
                        subtitle: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                Icon(Icons.star_rounded, color: avg > 0 ? Colors.amber : Colors.grey.shade400, size: 14),
                                const SizedBox(width: 4),
                                Text(avg > 0 ? avg.toStringAsFixed(1) : 'No ratings', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: avg > 0 ? Colors.amber.shade700 : Colors.grey.shade500)),
                                if (revCount > 0) Text(' ($revCount)', style: GoogleFonts.inter(fontSize: 12, color: AppColors.subtext)),
                              ],
                            ),
                            const SizedBox(height: 12),
                            LinearProgressIndicator(value: max > 0 ? count / max : 0, minHeight: 6, backgroundColor: AppColors.background, color: AppColors.primary, borderRadius: BorderRadius.circular(8)),
                          ],
                        ),
                      ),
                    );
  ...
  Widget _buildPieChartCard(String title, bool isOrderVolume) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: GoogleFonts.outfit(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.text)),
          const SizedBox(height: 16),
          if (!isOrderVolume && totalRating == 0)
            const SizedBox(height: 200, child: Center(child: Text('No ratings yet.')))
          else
            SizedBox(
              height: 250,
              child: SfCircularChart(
                ...
                legend: Legend(
                  ...
                  textStyle: GoogleFonts.inter(fontSize: 11, color: AppColors.text),
                ),
                  ...
                  CircularChartAnnotation(
                    widget: Column(
                      ...
                      children: [
                        ...
                        Text(isOrderVolume ? 'Orders' : 'Avg', style: GoogleFonts.inter(fontSize: 10, color: AppColors.subtext)),
                      ],
                    ),
                  ),
                ],
                series: <CircularSeries>[
                  DoughnutSeries<Map<String, dynamic>, String>(
                    ...
                    dataLabelSettings: DataLabelSettings(
                      ...
                      textStyle: GoogleFonts.inter(fontSize: 10, fontWeight: FontWeight.bold, color: AppColors.text),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const Spacer(),
          Text(value, style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.text)),
          const SizedBox(height: 4),
          Text(title, style: GoogleFonts.inter(color: AppColors.subtext, fontSize: 11, fontWeight: FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
